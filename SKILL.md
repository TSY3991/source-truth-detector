...
預設輸出純文字報告；可加 `--format json` 或 `--format md` 輸出 JSON / Markdown 格式，再加 `--output <file>` 寫入檔案（方便存檔或餵給其他工具）。
...
1. 確認使用者專案的根目錄（通常是 `src/`）以及實際的執行入口檔（如 `src/index.js`、`src/main.jsx`、`src/server.js`）。如果有多個入口（前端 + 後端 + worker），全部列出。如果該目錄已有 `.claude-truth-detector.json`，可省略 `--entry`，直接用設定檔裡的值。
2. 執行：
...
4. 如果使用者想存檔報告，可加 `--format json` / `--format md` 搭配 `--output <file>`。

5. 如果使用者想固定常用參數（entry / format / output / threshold），可以幫他在掃描目錄建立 `.claude-truth-detector.json`（schema 見 README），之後就不用每次重打參數。

6. **不要**根據 `UNREFERENCED` 結果自動刪除或修改任何檔案，除非使用者明確要求並再次確認。
