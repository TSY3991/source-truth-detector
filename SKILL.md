...
description: Read-only 依賴鏈分析工具 — 從指定的 entrypoint 出發，建立 import/require 依賴圖，分類專案內每個 .js/.jsx 檔案為 USED（被依賴鏈引用）、UNREFERENCED（追不到）、POSSIBLE_SOURCE_OF_TRUTH（高 fan-in，被大量檔案依賴）。絕不修改、刪除、重構任何檔案。觸發詞：「找出專案沒用到的檔案」「這個檔案還有人用嗎」「掃一下依賴關係」「找 dead code」「找 source of truth」「config_old / config_v2 哪個還在用」「source-truth-detector」「scan unreferenced files」時觸發。
---
